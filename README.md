...
- **Выбор модели** — Qwen3 235B или DeepSeek V3 2 через переключатель на главном экране
...
- **Дополнительные вопросы** — возможность дозадавать вопросы по результатам анализа в том же контексте
...
2. **Выберите модель** — Qwen3 235B или DeepSeek V3 2
3. **Загрузите CSV-файлы** или выберите из готовых датасетов
4. **Опишите аналитическую задачу** на естественном языке или выберите из примеров
5. **Наблюдайте** за генерацией и выполнением кода в реальном времени
6. **Получите результат**: текстовый анализ, графики, файлы для скачивания
7. **Задайте дополнительный вопрос** по результатам, если нужно уточнить или углубить анализ
...
curl "http://localhost:8000/api/analyze?query=Проанализируй+данные&file_ids=file_id_here&model=qwen3-235b-a22b-fp8/latest"
...
- **Модели**: Qwen3 235B и DeepSeek V3 2 через Yandex Cloud AI Studio (выбор на главном экране)
- **Code Interpreter**: выполнение Python-кода в изолированном облачном контейнере
- **Follow-up**: дополнительные вопросы через `previous_response_id` в том же контейнере

### Поток данных

1. Пользователь выбирает модель и загружает CSV → файл сохраняется в Yandex Cloud Files API
...
6. Пользователь может задать дополнительный вопрос → `previous_response_id` сохраняет контекст и контейнер
...
stream = await client.responses.create(
    ...
    previous_response_id=previous_response_id,  # для дополнительных вопросов
    stream=True,
)
```
