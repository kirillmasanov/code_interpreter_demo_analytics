...
- **Yandex Cloud AI Studio** — платформа для работы с AI-моделями и инструментами, включающая:
  ...
  - **Model Gallery** — доступ к языковым моделям (в проекте доступны Qwen3 235B и DeepSeek V3 2)
...
1. **Выбор модели**: Пользователь выбирает модель (Qwen3 235B или DeepSeek V3 2)
2. **Загрузка данных**: Пользователь загружает CSV-файлы или выбирает из готовых датасетов
3. **Аналитический запрос**: Пользователь описывает задачу на естественном языке
4. **Генерация кода**: Модель генерирует Python-код для анализа данных
5. **Выполнение кода**: Код выполняется в изолированном облачном контейнере с доступом к загруженным файлам
6. **Стриминг результатов**: Через SSE (Server-Sent Events) в реальном времени передаются код, логи выполнения и текстовый ответ
7. **Результат**: Отображение текстового анализа, графиков и файлов для скачивания
8. **Дополнительные вопросы**: Пользователь может задать уточняющие вопросы, сохраняя контекст и контейнер
...
- Выбор модели через UI (Qwen3 235B, DeepSeek V3 2)
- Дополнительные вопросы через `previous_response_id` в том же контейнере
...
2. Выберите модель для анализа (Qwen3 235B или DeepSeek V3 2)
3. Загрузите свой CSV-файл перетаскиванием или выбором через диалог
4. Или выберите один из готовых датасетов:
...
### 6.4. Дополнительные вопросы

После завершения анализа можно задать уточняющий вопрос:
1. Введите вопрос в поле под результатами (или нажмите Enter)
2. Код выполнится в том же контейнере с сохранением контекста предыдущего анализа
3. Новые результаты дописываются к существующим, а вопрос отображается как цитата
