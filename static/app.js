...
$btnNewQuery.addEventListener('click', () => { state.responseId = null; goToStep(2); });
...
/* ── Follow-up question ──────────────────────────────── */
$btnFollowup.addEventListener('click', startFollowUp);
$followupInput.addEventListener('keydown', e => {
  if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); startFollowUp(); }
});

function startFollowUp() {
  const query = $followupInput.value.trim();
  if (!query || !state.responseId) return;

  $followupSection.hidden = true;
  $followupInput.value = '';
  state._followUpQuery = query;

  $statusBar.className = 'status-bar';
  $statusText.textContent = 'Подключение к модели...';

  $codeOutput.textContent += '\n\n# ════════════════════════════════════════\n';
  $codeOutput.textContent += '# Дополнительный вопрос\n';
  $codeOutput.textContent += '# ════════════════════════════════════════\n';
...
/* ── SSE handler ──────────────────────────────────────── */
function handleSSE(evtSource, isFollowUp = false) {
  if (isFollowUp) state._prevTextHtml = $textContent.innerHTML;
  let textBuffer = isFollowUp ? `> **Вопрос:** ${state._followUpQuery}\n\n` : '';
  let hadDeltas = false;
  ...
  on('text_delta', d => {
    ...
    if (isFollowUp) {
      $textContent.innerHTML = state._prevTextHtml + '<hr class="followup-hr">' + marked.parse(textBuffer);
    } else {
  ...
  on('done', d => {
    ...
    if (d && d.response_id) state.responseId = d.response_id;
    state._prevTextHtml = $textContent.innerHTML;
